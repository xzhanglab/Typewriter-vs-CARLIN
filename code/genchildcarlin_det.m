...
function [y, nstp, nendp,nlivetar,nhotspots,nallele] = genchildcarlin_det(n,ldtl,stp,endp,parbarc,tarn,livetar, hotspot, mupb,lgdelprob,ins_sub,parallele)
% report detailed mutation types for each target for each cell
% k is division round
...
nallele = parallele; % temporarily copy parent's allele details
...
y=parbarc; % full parent barcode
                    ...
                    if (length(editedtar)>=2)% more than 2 cuts
                        ...
                        if (rand<lgdelprob) % perform one large deletion
                            ...
                            nallele(1,editedtar(min(randi)):editedtar(max(randi))) = 0; % indicate that those targets are no longer editable
                            nallele(2,editedtar(min(randi)):editedtar(max(randi))) = 1; % indicate that those targets have inter-target deletion
                            nallele(3,editedtar(min(randi)):editedtar(max(randi))) = 0; % it is possible that a certain target had a single cut mutation before, now wipe it out
     ...
     if totaldel>= totalins % no need to shift entries in y
        ...
        if totaldel== totalins
            if [ldel; rdel] ~= [lins; rins]% mutated target, set uneditable any more
                ...
                nallele(1,newedtar(i)) = 0; % indicate that this target has mutated
                nallele(3,newedtar(i)) = 1; % indicate that this target has intra-target indel
            end
        else % for sure it has mutated
            livetar(newedtar(i)) = 0;
            nallele(1,newedtar(i)) = 0; % indicate that this target has mutated
            nallele(3,newedtar(i)) = 1; % indicate that this target has intra-target indel
        end
